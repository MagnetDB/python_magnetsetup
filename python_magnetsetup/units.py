...
def load_units(distance_unit: str):
    ...
    units = {
        ...
        "Young": [
            ureg.kilogram / ureg.meter / ureg.second,
            ureg.kilogram / ureg.Unit(distance_unit) / ureg.second,
        ],
        "Length": [ureg.millimeter, ureg.Unit(distance_unit)],
        "Area": [
            ureg.millimeter * ureg.millimeter,
            ureg.Unit(distance_unit) * ureg.Unit(distance_unit),
        ],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Change units according to specify length unit"
    )
    parser.add_argument(
    ...
    with MyOpen(yamlfile, "r", paths=search_paths(MyEnv, "geom")) as cfgdata:
        ...
        if isinstance(cad, Insert):
            ...
            for data in [R1, R2, Z1, Z2, Zmin, Zmax, Dh]:
                _convert = convert_data(units, data, "Length")
